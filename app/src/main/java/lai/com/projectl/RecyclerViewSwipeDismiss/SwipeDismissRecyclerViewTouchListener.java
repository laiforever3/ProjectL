package lai.com.projectl.RecyclerViewSwipeDismiss;
...
public class SwipeDismissRecyclerViewTouchListener implements View.OnTouchListener {
    ...
    public SwipeDismissRecyclerViewTouchListener(Builder builder) {
        ...
        mCallbacks = builder.mDismissCallbacks;
    ...
    public boolean onTouch(View v, MotionEvent event) {
        ...
        switch (event.getActionMasked()) {
            case MotionEvent.ACTION_DOWN:
                ...
                int x = (int) (mDownX - location[0]);
                int y = (int) (mDownY - location[1]);
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mIsVertical) {
                    if(Math.abs(deltaY)>mSlop&&Math.abs(deltaY)/2>Math.abs(deltaX) ){
                       mSwiping = true;
            ...
            case MotionEvent.ACTION_UP:
                if (!mSwiping && mDownView != null && mItemTouchCallback != null&&mVelocityTracker!=null) {
                ...
                float deltaXX = event.getRawX() - mDownX;
                float deltaYY = event.getRawY() -mDownY;
                mVelocityTracker.addMovement(event);
                ...
                if (mIsVertical) {
                    if (Math.abs(deltaYY) > mViewWidth && mSwiping) {
                        dismiss = true;
                        dismissRight = deltaYY > 0;
                    } else if (absVelocityY > mMinFlingVelocity && absVelocityX < mMaxFlingVelocity && absVelocityY > absVelocityX && mSwiping) {
                        // dismiss only if flinging in the same direction as dragging
                        dismiss = (velocityY < 0) == (deltaYY < 0);
                    ...
                    if (Math.abs(deltaXX) > mViewWidth / 2 && mSwiping) {
                        dismiss = true;
                        dismissRight = deltaXX > 0;
                    } else if (mMinFlingVelocity <= absVelocityX && absVelocityX <= mMaxFlingVelocity
                            && absVelocityY < absVelocityX && mSwiping) {
                        // dismiss only if flinging in the same direction as dragging
                        dismiss = (velocityX < 0) == (deltaXX < 0);
    ...
    private void performDismiss(final View dismissView, final int dismissPosition) {
        ...
        animator.addListener(new AnimatorListenerAdapter() {
            @Override
            public void onAnimationEnd(Animator animation) {
                Log.e("lai", "mDismissAnimationRefCount=" + mDismissAnimationRefCount);
                --mDismissAnimationRefCount;
                Log.e("lai", "mDismissAnimationRefCount=" +mDismissAnimationRefCount);
